import { PriceAlert, Product, User } from "@prisma/client";
...
export async function checkPriceAlerts(productId: string, newPrice: number) {
  try {
    ...
    logger.info(
      `Verificando alertas para producto ${productId}: ${alerts.length} alertas encontradas`
    );
...
async function sendPriceAlertNotification(
  alert: PriceAlert & { user: User } & { product: Product }
) {
