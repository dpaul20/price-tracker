...
// Programador para trabajos recurrentes
const scheduler = new QueueScheduler("price-updates", { connection })
...
const priceUpdateWorker = new Worker(
  ...
  async (job) => {
    ...
    try {
      logger.info(`Procesando actualización de precio para producto ${productId}`)
      ...
      if (newPrice !== product.currentPrice) {
        logger.info(`Precio cambiado para ${productId}: ${product.currentPrice} -> ${newPrice}`)
        ...
        // Verificar alertas
        const triggeredAlerts = await priceAlertRepository.checkAlertsForProduct(productId, newPrice)

        if (triggeredAlerts.length > 0) {
          logger.info(`Se activaron ${triggeredAlerts.length} alertas para el producto ${productId}`)
        }
      ...
      logger.error(`Error procesando producto ${productId}: ${(error as Error).message}`)
      throw error
    }
  },
...
export async function scheduleProductUpdates(batchSize = 50) {
  try {
    ...
    // Sort by lastChecked
    products.sort((a, b) => new Date(a.lastChecked).getTime() - new Date(b.lastChecked).getTime())

    logger.info(`Programando actualizaciones para ${products.length} productos en lotes de ${batchSize}`)
    ...
    logger.error(`Error programando actualizaciones: ${(error as Error).message}`)
    throw error
  }
}
...
function calculatePriority(product: any): number {
  const now = new Date()
  const hoursSinceLastCheck = (now.getTime() - new Date(product.lastChecked).getTime()) / (1000 * 60 * 60)
